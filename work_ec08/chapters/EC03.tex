...
\]
where $\rho$ is air density, $A$ the swept area and $v$ wind speed. The cubic dependence on $v$ is essential: doubling wind speed multiplies the available power by eight. This is why careful site measurement and long-term wind statistics are more important than a single instantaneous reading.
...
A wind turbine does not produce its rated power all the time. Below the cut-in speed, it remains stopped. Between cut-in and rated speed, output rises rapidly. Above rated speed, control systems limit the power. At the cut-out speed, the turbine shuts down to avoid excessive mechanical loads.
\end{ecarticle}
...
No turbine can capture all the power in the wind. The theoretical upper limit is the \textbf{Betz limit}: 59.3\% of the kinetic power crossing the rotor area.
\end{tcolorbox}
...
\foreach \a in {0,120,240}{\draw[very thick,ECGreen,rotate around={\a:(-1.45,3.75)}] (-1.45,3.75)--(-1.45,5.0);}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\linewidth,height=47mm,xlabel={Wind speed (m/s)},ylabel={Power (MW)},xmin=0,xmax=26,ymin=0,ymax=3.3,grid=major,tick label style={font=\scriptsize},label style={font=\scriptsize}]
\addplot[very thick,ECBlue,smooth] table[x=wind,y=power,col sep=comma]{data/EC03/wind_power_curve.csv};
...
A wind turbine has a rotor diameter of 100~m and operates in air of density $1.225~\mathrm{kg\,m^{-3}}$ at a wind speed of 10~m/s. Using $P=\tfrac12\rho A v^3 C_p$ with $C_p=0.44$, estimate the aerodynamic power captured by the rotor. Compare $C_p$ with the Betz limit, then estimate the electrical power delivered to the grid if the mechanical and electrical efficiency is 90\%.
