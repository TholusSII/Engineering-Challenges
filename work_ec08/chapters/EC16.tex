...
\ECObjectives{
\begin{itemize}
\item Distinguish confidentiality, integrity and availability.
\item Explain defence in depth for a connected engineering system.
\item Analyse technical risk using threats, vulnerabilities and consequences.
\end{itemize}}

\begin{multicols}{2}
\begin{engineeringnews}
Industrial systems, vehicles, hospitals and energy networks increasingly depend on connected
software. Cybersecurity is therefore a safety and business-continuity issue as well as an information-technology
issue: a digital attack can produce physical consequences.
\end{engineeringnews}
...
Risk depends on more than the mere existence of a threat. Engineers identify valuable assets, possible
attackers, vulnerabilities and consequences. A weak password on a non-critical display is not the
same risk as an unprotected remote command to a water-treatment pump. Safety analysis and cyber-risk
analysis must therefore be coordinated.
...
tool added at the end. Human factors matter too: clear procedures and training reduce errors and social
engineering attacks.
\end{ecarticle}
...
access control should prevent intrusion. The final design must satisfy both requirements.
\end{tcolorbox}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ybar,width=\linewidth,height=51mm,ylabel={Residual risk index},ymin=0,ymax=110,
...
zero and each layer must be maintained.
\end{tcolorbox}
...
\end{center}
Safeguards may reduce the likelihood of the event, limit its consequences, or improve detection and
recovery.
\end{tcolorbox}
...
\item A greenhouse must continue watering crops during a network outage. Propose one local fail-safe
mode and one method for recording the incident.
\end{enumerate}
...
Identify assets, one threat and one vulnerability. Propose authentication, segmentation, monitoring
and a safe local mode if the network is unavailable.
\end{engineeringchallenge}

\begin{speaking}
Is a fully connected system always better than an isolated one? Compare functionality, maintenance,
security and resilience using one engineering example.
\end{speaking}
\end{multicols}
