...
\ECObjectives{
\begin{itemize}
\item Describe the energy and information flows in a battery-electric vehicle.
\item Explain charging, regenerative braking and battery thermal management.
\item Evaluate an electric-mobility project using technical, economic and environmental criteria.
\end{itemize}}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=.96\linewidth,height=45mm,ybar,bar width=13pt,ymin=0,ymax=100,ylabel={Energy reaching the wheels (kWh)},symbolic x coords={Battery EV,Hydrogen EV,Petrol car},xtick=data,xticklabel style={font=\scriptsize,align=center},yticklabel style={font=\scriptsize},grid=major,nodes near coords,every node near coord/.append style={font=\scriptsize}]
\addplot[fill=ECBlue] table[x=vehicle,y=wheel_energy,col sep=comma]{data/EC06/energy_pathways.csv};
\end{axis}
\end{tikzpicture}
\end{center}
\footnotesize Illustrative conversion chain starting from 100~kWh of supplied energy. Actual values vary with technology, driving conditions and system boundaries.
\end{tcolorbox}
...
\item Use the graph to compare the three energy pathways.
\item Why is the largest possible battery not always the best design choice?
\end{questions}
