...
\item Identify the sensors used for stabilisation and navigation.
\item Discuss endurance, payload, regulation and operational safety.
\end{itemize}}
...
position and horizontal velocity. Sensor fusion is essential because every sensor has limitations: gyroscopes drift,
accelerometers are disturbed by vibration and GNSS signals may be inaccurate or unavailable indoors.
...
so the motors must generate additional thrust to maintain altitude.
\end{tcolorbox}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=\linewidth,height=49mm,xlabel={Payload (kg)},ylabel={Endurance (min)},xmin=0,xmax=1.2,
...
\end{tikzpicture}
\end{center}
\footnotesize Illustrative trend for one aircraft: endurance decreases non-linearly as payload increases.
\end{tcolorbox}
...
\item State two reasons why the pilot should use a larger reserve in strong wind.
\end{enumerate}
...
Define a drone mission to inspect a bridge. Estimate the required payload and flight time, select the
main sensors, propose a safe path and define the response to low battery, GNSS loss and communication
loss.
